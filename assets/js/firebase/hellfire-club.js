...
export async function getHellfireClubSubscriptions() {
    const db = getFirestore(app)
    const hellfireClubCollection = collection(db, 'hellfire-clube')
    const getHellfireClubCollectionSnapshot = await getDocs(hellfireClubCollection);
    const subscriptions = getHellfireClubCollectionSnapshot.docs.map(doc => doc.data());
